...
public class SqlBuilder {
	...
	public SqlBuilder like(SSupplier<String> val) {
		sql.append(" and ").append(SqlBuilderUtils.getColumn(beanClass, val)).append(" like ?");
		param.add("%" + val.get() + "%");
		return this;
	}
	...
	public SqlBuilder notLike(SSupplier<String> val) {
		sql.append(" and ").append(SqlBuilderUtils.getColumn(beanClass, val)).append(" not like ?");
		param.add("%" + val.get() + "%");
		return this;
	}

	public SqlBuilder notLike(String column, Object val) {
		sql.append(" and ").append(column).append(" not like ?");
		param.add("%" + val + "%");
		return this;
	}
	...
	public <T> SqlBuilder between(String column, Supplier<T> leftVal, Supplier<T> rightVal) {
		sql.append(" and ").append(column).append(" between").append(" ? and ?");
	...
	public <T> SqlBuilder notBetween(String column, Supplier<T> leftVal, Supplier<T> rightVal) {
		sql.append(" and ").append(column).append(" not between").append(" ? and ?");
		param.add(leftVal.get());
		param.add(rightVal.get());
		return this;
	}
	...
	public <T> SqlBuilder and(Consumer<SqlBuilder> lambdaWhereBuilder) {
		...
		// 5代表" and "的长度
		sql.append(" and (").append(lq.getSql().substring(5)).append(")");
	...
	public <T> SqlBuilder or(Consumer<SqlBuilder> lambdaWhereBuilder) {
		...
		lambdaWhereBuilder.accept(lq);
		// 5代表" and "的长度
		sql.append(" or (").append(lq.getSql().substring(5)).append(")");
		param.addAll(lq.getParam());
		return this;
	}
	...
	public <T> SqlBuilder in(SSupplier<T> val) {
		...
		if (cls instanceof Collection<?>) {
			...
			sql.append(" and ").append(SqlBuilderUtils.getColumn(beanClass, val)).append(" in (");
	...
	public <T> SqlBuilder notIn(SSupplier<T> val) {
		...
		if (cls instanceof Collection<?>) {
			Collection values = (Collection) cls;
			sql.append(" and ").append(SqlBuilderUtils.getColumn(beanClass, val)).append(" not in (");
			values.stream().forEach(e -> {
				if (e instanceof Number) {
					param.add(e);
				}
