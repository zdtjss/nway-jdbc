 ...
 *
 * <p>
 * 注：本类使用了{@link org.springframework.jdbc.core.BeanPropertyRowMapper}源码，新增了通过
 * {@link com.nway.spring.jdbc.annotation.Column}自定义表字段与bean属性的映射
 * 
...
public class BeanPropertyRowMapper<T> implements RowMapper<T> {
	...
	private String annotationName(PropertyDescriptor pd) {
		...
		if (columnAnnotation != null) {
			...
			String columnName = columnAnnotation.value() == null ? columnAnnotation.name() : columnAnnotation.value();
					...
					try {
						...
						if (value == null && this.primitivesDefaultedForNullValue) {
							logger.debug("Intercepted TypeMismatchException for row " + rowNumber + " and column '"
									+ column + "' with null value when setting property '" + pd.getName() + "' of type "
